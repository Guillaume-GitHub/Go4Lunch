...
public class RestoDetailsClient {
    ...
    public Observable<DetailsRestaurant> allDetailsRestaurant(String placeId) {
        map = new HashMap<>();
        map.put("placeid", placeId);
        map.put("fields", place_id + "," + name + "," + vicinity + "," + rating + "," + photo + "," + website + "," + formattedNumber);
        ...
        return detailsRestaurant(placeId)
                .map(new Function<DetailsRestaurant, OpeningHours>() {
                    ...
                    public OpeningHours apply(DetailsRestaurant detailsRestaurant) throws Exception {
                        return detailsRestaurant.getDetailsRestaurantResult().getOpeningHours();
                    }
                });
    }
}
